...
(function () {
    ...
    window.Promo = {
        ...
        initControl: function () {
            ...
            //标题hover事件
            this.promoTitleSpan.on('mouseover', function () {
    ...
    var NewsJs = {
        ...
        delayRunFunc: function () {
            ...
            new L_CommTab({
                ...
                changeStartBack: function ($title) {
                    this.changeNewsTab($title.attr('data-newsId'));
                }.bind(this)
            });
        },
        /**获取拼接后的api*/
        ...
        /**请求成功之后,填充数据*/
        fullNews: function (newsId, newsData) {
            //对比newsId是否与现在被选中的id一致
            if (this.nowShowId === newsId) {
                this.newsListContainer.html(template('J_newsItemTemplate', newsData));
            }
        }
    };
    ...
    var ActionJs = {
        ...
        checkLoadOver: function () {
            if (window['action'] && window['match']) {
                ...
                for (var i = 0, j = this.All.length; i < j; ++i) {
                    temp = this.All[i];
                    if (+temp.iDate === 0 && +temp.iStatus === 999) {
                        this.CQHD.push(temp)
                    }else if(this.culRemainTime(temp) > 0){
                        this.ZZJX.push(temp);
                    }
                }
                //绑定操作事件,初始化tab点击
        ...
        handleData: function (tempArray) {
            ...
            for (var i = 0, j = tempArray.length; i < j; ++i) {
                tempOne = tempArray[i];
                //计算结束提示
                tempOne.remainTip = this.culOverTimeTip(tempOne);
                //计算是否是new
                tempOne.isNew = this.checkNewAct(tempOne);
                //计算长期活动
                if (+tempOne.iDate === 0 && +tempOne.iStatus === 999) {
        ...
        fullAction: function (data) {
            $('#J_actListContainer').html(template('J_actItemTemplate', {list: data}));
        }
    };
    /**  版本更新部分  **/
    var NewVersion = {
        ...
        fullData: function () {
            ...
            if (OfficialWebsiteCfg['champion']) {
                ...
                $('#J_newChampionContainer').html(template('J_newChampionTemplate', champion));
                this.getAllChampionData();
            }
            //新皮肤
            if (OfficialWebsiteCfg['skin']) {
                ...
                $('#J_newSkinContainer').html(template('J_newSkinTemplate',mainSkin));
            ...
            if (OfficialWebsiteCfg['version']) {
                ...
                $('#J_newVersionContainer').html(template('J_newVersionTemplate', version));
                hoverPlayInnerVideo('#J_newVersionContainer');
            }
            //新手办
            if (OfficialWebsiteCfg['peripheral']) {
                var peripheral = OfficialWebsiteCfg.peripheral;
                $('#J_newModelContainer').html(template('J_newModelTemplate', peripheral));
            }
            //开发者
            if (OfficialWebsiteCfg['developer']) {
                var developer = OfficialWebsiteCfg.developer;
                $('#J_clubDeveloperContainer').html(template('J_clubDeveloperTemplate', developer));
            }
        },
        /**填充周免数据*/
        ...
        /**读取英雄列表以便查询新英雄属性**/
        getAllChampionData: function () {
            //判断全体英雄列表数据是否加载过
            if (window['LOLherojs'] && LOLherojs['champion']) {
                //加载过,进行新英雄数据处理
                this.searchChampionInfo();
            } else {
                var requestParam = {
                    apiUrl: '//lol.qq.com/biz/hero/champion.js?v=' + L_CommFunc.ran,
                    $requestType: 'getScript',
                    successBack: function () {
                        try {
                            if (LOLherojs && LOLherojs.champion) {
                                NewVersion.searchChampionInfo();
                            } else {
                                //再次请求
                                this.requestOne();
                            }
        ...
        /**寻找新英雄属性数据*/
        searchChampionInfo: function () {
            //获取新英雄id
            var championId = +$('#J_championAbility').attr('data-championId');

            //获取key
            var key = LOLherojs.champion.keys[championId];

            //加载属性数据
            var requestParam = {
                apiUrl: '//lol.qq.com/biz/hero/' + key + '.js?v=' + L_CommFunc.ran,
                $requestType: 'getScript',
                successBack: function () {
                    try {
                        if (LOLherojs && LOLherojs.champion && LOLherojs.champion[key]) {
                            NewVersion.setChampionInfo(LOLherojs.champion[key].data);
                        } else {
                            //再次请求
                            this.requestOne();
                        }
        ...
        /**设置新英雄属性*/
        setChampionInfo: function (data) {
            var info = data.info;
            var championAbility = $('#J_championAbility');
            championAbility.find('#J_attackBar').css('width', info.attack * 10 + '%');
            championAbility.find('#J_magicBar').css('width', info.magic * 10 + '%');
            championAbility.find('#J_defenseBar').css('width', info.defense * 10 + '%');
            championAbility.find('#J_hardBar').addClass('h' + Math.round(info.difficulty * 3 / 10));
        },
        /**绑定交互事件*/
        bindEvent: function () {
            //显示更多新皮肤
        ...
        cutVid: function (txvideourl) {
            var result = txvideourl.match('page/(\\S*).h');
            if (result) {
                return result[1];
            } else {
                return false;
    ...
    var VideoProgram = {
        ...
        programProcess: {
            ...
            init: function () {
                ...
                var programTab = new L_CommTab({
                    ...
                    //tab切换回调事件
                    changeStartBack: function ($title) {
                        this.changeProgramDay($title.attr('data-programDay'));
                    }.bind(this)
                });
                //判断当前是星期几,进入今天的专辑
        ...
        videoProcess: {
            /*最新视频数据缓存,避免重复请求*/
            flashVideoDataCache: {},
            /*最后一次切换请求需要填充的数据标识,填充时判断,以免多个异步请求填充时造成错乱,要求每次切换都将标识沿着作用域链传递下去.*/
            lastFullFlashVideo: null,
            /*初始化交互操作*/
            init: function () {
                //根据配置信息填充标题,"推荐"接口不一样需要注意,其他的都用分类的"最新"id
                var titleLi = '<li data-tabvalue="recommend">推荐</li>';
                var temptags;
                for (var i = 0, j = VideoCenterCfg.tags.length; i < j; ++i) {
                    temptags = VideoCenterCfg.tags[i];
                    titleLi += "<li data-tabvalue='" + temptags.cates[0].value + "'>" + temptags.name + "</li>";
                }
                ...
                new L_CommTab({
                    ...
                    //tab切换回调事件
                    changeStartBack: function ($title) {
                        this.changeFlashVedio($title.attr('data-tabvalue'));
                    }.bind(this)
                });
            },
            /*切换最新视频tab响应方法*/
            changeFlashVedio: function (tabvalue) {
                //记录数据类型
                this.lastFullFlashVideo = tabvalue;
                //进入请求流程
                this.requestVedioList(tabvalue);
            },
            /*请求列表数据*/
            requestVedioList: function (tabvalue) {
                addLoadingTip('ing','#J_flashVideoContainer','li',true);
                //判断是否有缓存
                if (this.flashVideoDataCache[tabvalue]) {
                    //存在缓存,直接填充
                    this.fullVideo(tabvalue, this.flashVideoDataCache[tabvalue]);
                } else {
                    var apiUrl;
                    if (tabvalue === 'recommend') {
                        ...
                        apiUrl = '//apps.game.qq.com/wmp/v3.1/?p0=3&p1=searchKeywordsList&page=1&pagesize=8&order=sIdxTime&type=iTag&id=' + tabvalue + '&r0=jsonp&source=web_pc';
                    }
                    var requestParam = {
                        apiUrl: apiUrl,
                        $requestType: 'ajax',
                        ...
                        successBack: function (e) {
                            try {
                                if (e.msg === 'OK') {
                                    ...
                                    this.attach.requestVedioListSuccess(tabvalue, e.data);
                                } else if (+e.status === 0) {
                                    //其他几个分页请求成功,交由attach引用programProcess的成功函数处理
                                    this.attach.requestVedioListSuccess(tabvalue, e.msg);
                                }
                                else {
                                    //再次请求
                                    this.requestOne();
            ...
            requestVedioListSuccess: function (tabvalue, listData) {
                //截断为8个
                ...
                //放入缓存
                this.flashVideoDataCache[tabvalue] = listData;
                //填充
                this.fullVideo(tabvalue, listData);
            ...
            /*填充*/
            fullVideo: function (tabvalue, listData) {
                if(this.lastFullFlashVideo !== tabvalue)return;
        ...
        hoverImg:{
            ...
            handleMousemove:function(event,dom){
                ...
                var $dom = $(dom),vid = $dom.data('vid'),data = this.dataCache[vid];
                //判断是否存在数据
                if(typeof data === 'undefined' || +data.retcode !== 0)return;
                data = data.fields.smart_pic_infos[this.sizeKey];
                var domWidth = $dom.width(),imgLength = data.length;
            ...
            handleData:function (result) {
                ...
                for(var i=0,j=result.length;i<j;++i){
                    ...
                    fields.smart_pic_infos = $.parseJSON(fields.smart_pic_infos);
     ...
     * **/
    var EventJs = {
        matchId:110,//官网每次只显示一种赛事，这个赛事可能分为很多子赛事，如常规赛，季后赛等等，由gameType决定。
        ...
        getEventNewsSuccess: function (data) {
            ...
            news.attr('href', innerData.sUrl).attr('onclick', "pgvSendClick({hottag:'v3.eventtopnews'})").css('display', 'block');
        },
        /*切换赛事tab*/
    ...
    var ChampionData = {
        ...
        fullChampionData: function () {
            ...
            container.html(template('championItem', LOLherojs.champion));
        ...
        monitorSort: function () {
            ...
            tempLi.on('click', function () {
                ...
                ChampionData.showTagChampion($(this).addClass('selected').attr('data-sort'));
            });
        },
        /**显示某个标签的英雄,隐藏其他英雄*/
    ...
    var FanartJs = {
        ...
        adaptFanartImg:function(img){
            ...
            if(n_imgWidth>n_imgHeight){
                fanartImg.css({height:'176px',width:'auto'});
                r_imgHeight = 176;
                r_imgWidth = Math.ceil(176/n_imgHeight * n_imgWidth);
            }else{
                fanartImg.css({width:'192px',height:'auto'});
                r_imgWidth = 192;
        ...
        requestZan: function (e) {
            var self = FanartJs;
            var tempTarget = $(e.target), iContentId, zanBtn;
